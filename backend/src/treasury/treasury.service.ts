...
export class TreasuryService {
  ...
  async getSummary(dateFrom?: string, dateTo?: string) {
    ...
    // Supplier payments — normalize legacy Arabic values + any free-text methods
    for (const row of supplierOut) {
      const key = this.normalizeMethod(row.method);
  ...
  async getTransactions(query: TreasuryQueryDto) {
    ...
    const allEntries: any[] = [
      ...
      ...supplierPayments.map((sp) => ({
        ...
        paymentMethod: this.normalizeMethod(sp.method),
