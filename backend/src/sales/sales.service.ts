...
export class SalesService {
  ...
  async createSale(createSaleDto: CreateSaleDto, userId: number) {
    ...
    // Create sale in transaction with retry for invoice number conflicts
    const MAX_RETRIES = 3;
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        return await this.prisma.$transaction(async (tx) => {
      // Generate invoice number inside transaction
      const invoiceNo = await this.generateInvoiceNo(branchId, tx);
      ...
      return invoice;
        ...
        // Retry on unique constraint violation (P2002) for invoice number
...
`;
  ...
  async getAllCustomersWithDebt() {
    ...
    return customers.map((c) => {
      const totalDebt = c.salesInvoices.reduce(
        (sum, inv) => sum + Number(inv.remainingAmount),
        0,
      );
      const totalPurchases = c.salesInvoices.reduce(
        (sum, inv) => sum + Number(inv.total),
        0,
      );
      const invoiceCount = c.salesInvoices.length;
      const pendingCount = c.salesInvoices.filter(
        (inv) => inv.paymentStatus !== 'PAID',
      ).length;
      const { salesInvoices: _invoices, ...rest } = c;
      return {
        ...rest,
  ...
  async getCustomerStatement(
    ...
    const customer = await this.prisma.customer.findUnique({
      where: { id: customerId },
      select: { id: true, name: true, phone: true, type: true, address: true },
    });
    ...
    const mapped = invoices.map((inv) => ({
      ...
      shippingFee: Number(inv.shippingFee),
      ...
      lines: inv.lines.map((line) => ({
        ...
        productName:
          line.product?.nameAr || line.product?.nameEn || 'منتج غير معروف',
        barcode: line.product?.barcode || '',
      })),
    }));

    const totalPurchases = mapped.reduce((s, i) => s + i.total, 0);
    const totalPaid = mapped.reduce((s, i) => s + i.paidAmount, 0);
    const totalDebt = mapped.reduce((s, i) => s + i.remainingAmount, 0);
    ...
    return {
      ...
      summary: {
        ...
        totalDebt: parseFloat(totalDebt.toFixed(2)),
        paidInvoices: mapped.filter((i) => i.paymentStatus === 'PAID').length,
        partialInvoices: mapped.filter((i) => i.paymentStatus === 'PARTIAL')
          .length,
        unpaidInvoices: mapped.filter((i) => i.paymentStatus === 'UNPAID')
          .length,
        undelivered: mapped.filter((i) => !i.delivered).length,
      },
    };
  }
  ...
  private async generateInvoiceNo(branchId: number, prisma?: any): Promise<string> {
